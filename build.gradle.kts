...
// build script function captures the build script, which the configuration cache cannot serialize.
object JavadocFonts {
...
dependencies { add("requireJavadoc", libs.require.javadoc) }
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    // A local variable, because a task action that reads a build script property captures the
